...
class Forvo:
    ...
    def word(self, w):
        ...
        # results = soup.select(
        #     "#language-container-de>article>ul.show-all-pronunciations>li:not(.li-ad)"
        # )
        results = soup.select(
            "#language-container-de>article>#pronunciations-list-de>li:not(.li-ad)"
        )
        audio_sources = []
        for i in results:
